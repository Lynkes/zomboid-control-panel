...
(2026-08-23). The commands below preserve the reusable parts of those
audits as a worked example.
...
confidence. A 4th flagged command (`reloadlua`) turned out on manual
inspection to be a plausible false positive (the loop is list housekeeping,
...
verification half -- the pattern is documented in the original audit notes,
not shipped as a ready-to-run script,
