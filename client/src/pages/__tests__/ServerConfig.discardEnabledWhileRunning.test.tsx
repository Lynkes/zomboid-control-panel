...
describe('ServerConfig.tsx: Discard is not gated on serverMayBeRunning (it never calls the server)', () => {
  it('stays enabled while the server may be running, and clicking it does not attempt a save', async () => {
    ...
    // (Separately, and NOT part of this fix: hasIniChanges compares
    // rawContent/originalRawContent while in raw mode, and
    // discardIniChanges() only ever resets `iniSettings` -- raw-mode
    // discard not clearing the "Unsaved changes" badge is a real, distinct
    // gap this test incidentally surfaced but does not fix, out of scope
    // for "the button must not be disabled while the server may be
    // running.")
