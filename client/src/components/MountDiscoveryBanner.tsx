  ...
  return (
    <div className="flex flex-wrap items-center gap-3 rounded-md border bg-muted/10 px-3 py-2">
      <div className="flex min-w-0 flex-1 items-center gap-2">
        ...
        <span className="text-sm font-medium">PZ install</span>
        <code className="min-w-0 truncate font-mono text-xs text-muted-foreground" title={mount.installPath}>
          {mount.installPath}
        </code>
      </div>
      <div className="flex shrink-0 items-center gap-1">
        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={dismiss} aria-label="Dismiss discovered install" title="Dismiss">
          <X className="h-4 w-4" />
        </Button>
